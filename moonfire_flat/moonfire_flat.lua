...
version: 1.0.0
description: >-
  Jump up the mast walk, hotkey MUST have default keybind of "/"
plugin_dependencies:
- vnavmesh
...
clear_speed_buffs()
set_walk(false)
WalkTo(771.4, 37.4, 438.4, nil, false)
jump_to_point({ 772.9, 38.3, 436.2 }, .2)
set_walk(true)
jump_to_point({ 774.2, 39.4, 436.8 }, .2)
move_to_point({ 774.7, 39.4, 436.8 })
jump_to_point({ 776.0, 40.8, 435.9 }, .2)
move_to_point({ 775.8, 40.8, 435.7 })
jump_to_point({ 776.3, 42.0, 434.2 }, .2)
jump_to_point({ 775.7, 43.3, 432.5 }, .2)
jump_to_point({ 773.6, 44.3, 431.8 }, .2)
jump_to_point({ 772.5, 45.7, 433.0 }, .2)
move_to_point({ 772.4, 45.7, 432.6 })
jump_to_point({ 772.2, 47.2, 434.1 }, .2)
set_walk(false)
jump_to_point({ 772.0, 47.9, 438.7 }, .2)

set_walk(false)
move_to_point({ 772.3, 47.9, 438.5 })
start_sprint()
wait_ready(1, .2, true, .1)
jump_to_point({ 778.3, 48.8, 438.8 }, .5)
set_walk(true)

move_to_point({ 778.3, 48.8, 438.6 })
jump_to_point({ 777.1, 49.7, 436.4 }, .2)
jump_to_point({ 775.1, 50.6, 436.3 }, .2)
set_walk(false)
clear_speed_buffs()
jump_to_point({ 773.6, 52.3, 439.1 }, .2)
jump_to_point({ 777.2, 53.4, 438.4 }, .2)


set_walk(true)
jump_to_point({ 775.6, 54.5, 436.5 }, .2)
set_walk(false)
move_to_point({ 775.7, 54.5, 435.7 })
jump_to_point({ 773.4, 55.7, 439.2 }, .2)
move_to_point({ 773.8, 55.7, 439.2 })
jump_to_point({ 777.5, 57.3, 438.5 }, .4)

move_to_point({ 777.1, 57.3, 437.2 })
set_walk(true)
jump_to_point({ 775.2, 58.2, 437.8 }, .2)
jump_to_point({ 773.7, 58.8, 438.7 }, .2)
--middle mast
set_walk(false)
move_to_point({ 771.3, 58.8, 439.0 })

jump_to_point({ 766.3, 59.8, 437.8 }, .4)
jump_to_point({ 766.1, 60.8, 433.1 }, .2)
jump_to_point({ 766.7, 62.2, 429.1 }, .4)
jump_to_point({ 766.8, 63.8, 433.3 }, .2)
move_to_point({ 766.8, 63.9, 433.5 })
jump_to_point({ 770.2, 65.0, 430.8 }, .2)

--T1
set_walk(true)
jump_to_point({ 771.0, 66.4, 432.1 }, .4)
jump_to_point({ 772.6, 67.4, 433.4 }, .3)
jump_to_point({ 771.5, 68.2, 431.8 }, .3)

--T2
jump_to_point({ 771.6, 69.9, 430.4 }, .3)
jump_to_point({ 773.7, 70.9, 430.0 }, .3)
move_to_point({ 773.2, 70.9, 430.0 })

--T3
set_walk(false)
jump_to_point({ 776.6, 71.9, 432.1 }, .5)
set_walk(true)
jump_to_point({ 777.4, 72.9, 433.9 }, .3)
jump_to_point({ 777.1, 73.7, 432.0 }, .3)

jump_to_point({ 778.4, 75.2, 432.6 }, .3)
jump_to_point({ 778.5, 76.2, 434.1 }, .3)
move_to_point({ 777.6, 76.2, 434.6 })

set_walk(false)
jump_to_point({ 777.9, 77.6, 439.2 }, .4)
move_to_point({ 778.2, 77.6, 439.7 })
set_walk(true)
jump_to_point({ 777.1, 78.7, 441.0 }, .3)

set_walk(false)
jump_to_point({ 772.7, 79.3, 444.2 }, .5)
set_walk(true)
jump_to_point({ 771.4, 80.4, 444.8 }, .3)
set_walk(false)
jump_to_point({ 766.2, 81.5, 442.4 }, .5)
set_walk(true)
jump_to_point({ 765.3, 82.5, 441.6 }, .3)
move_to_point({ 765.0, 82.5, 441.0 })
set_walk(false)
jump_to_point({ 770.4, 82.5, 436.6 }, .5)
